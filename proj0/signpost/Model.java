...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        for (int c = 0; c < width(); c++) {
            for (int r = 0; r < height(); r++) {
                if (solution[c][r] == 1) {
                    _board[c][r] = new Sq(c, r, 1,
                            true, arrowDirection(c, r), 0);
                } else if (solution[c][r] == size()) {
                    _board[c][r] = new Sq(c, r, size(),
                            true, arrowDirection(c, r), 0);
                } else {
                    _board[c][r] = new Sq(c, r, 0,
                            false, arrowDirection(c, r), -1);
                }
                _solnNumToPlace[solution[c][r]] = pl(c, r);
        ...
        for (Sq[] row : _board) {
            for (Sq square : row) {
                ...
                for (Place successor : square._successors) {
                    if (_board[successor.x][successor.y]._predecessors
                            == null) {
                        _board[successor.x][successor.y]._predecessors
                                = new PlaceList();
                    }
                    _board[successor.x][successor.y]._predecessors.add(
                            _solnNumToPlace[solution[square.x][square.y]]);
                }
            }
        }
        _unconnected = last - 1;
    ...
    Model(Model model) {
        ...
        _board = new Sq[model.width()][model.height()];
        for (int x = 0; x < model.width(); x++) {
            for (int y = 0; y < model.height(); y++) {
                _board[x][y] = new Sq(x, y, model._board[x][y].sequenceNum(),
                        model._board[x][y].hasFixedNum(),
                        model._board[x][y].direction(),
                        model._board[x][y].group());
                _allSquares.add(_board[x][y]);
            }
        }
        for (int x = 0; x < model.width(); x++) {
            for (int y = 0; y < model.height(); y++) {
                Sq copyFrom = model._board[x][y];
                if (copyFrom.successor() != null) {
                    _board[x][y]._successor = _board[copyFrom.x][copyFrom.y];
                }
                if (copyFrom.predecessor() != null) {
                    _board[x][y]._predecessor = _board[copyFrom.x][copyFrom.y];
                }
                _board[x][y]._head =
                        _board[copyFrom.head().x][copyFrom.head().y];
            }
        }
    }
    ...
    boolean autoconnect() {
        ...
        for (Sq first : _allSquares) {
            for (Sq second : _allSquares) {
                if (first.sequenceNum() == second.sequenceNum() - 1) {
    ...
    void solve() {
        for (Sq sq: _allSquares) {
            sq._sequenceNum = solution()[sq.x][sq.y];
        }
        autoconnect();
        _unconnected = 0;
    }
    ...
    private int arrowDirection(int x, int y) {
        ...
        if (seq0 < size()) {
            ...
            for (int c = 0; c < width(); c++) {
                for (int r = 0; r < height(); r++) {
                    if (_solution[c][r] == seq0 + 1) {
                        sol = pl(c, r);
                    }
                }
            }
            PlaceList[] pListArr = _allSuccessors[x][y];
            for (int i = 1; i < pListArr.length; i++) {
                if (pListArr[i].contains(sol)) {
                    return i;
                }
            }
        }
    ...
    final class Sq {
        ...
        boolean connectable(Sq s1) {
            if (_allSuccessors[x][y][direction()].contains(pl(s1.x, s1.y))
                    && s1.predecessor() == null && this.successor() == null
                    && this.sequenceNum() != size() && s1.sequenceNum() != 1) {
        ...
        boolean connect(Sq s1) {
            ...
            boolean thisunnum  = this.sequenceNum() == 0;
            boolean s1unnum  = s1.sequenceNum() == 0;
            if (this.sequenceNum() != 0) {
                int num = this.sequenceNum() + 1;
                Sq next = this.successor();
                while (next != null) {
                    next._sequenceNum = num;
                    next = next.successor();
                    num += 1;
                }
            } else if (s1.sequenceNum() != 0) {
                int num = s1.sequenceNum() - 1;
                Sq last = s1.predecessor();
                while (last != null) {
                    last._sequenceNum = num;
            ...
            if (thisunnum && this.sequenceNum() != 0) {
                releaseGroup(this._head._group);
            }
            if (s1unnum && s1.sequenceNum() != 0) {
                releaseGroup(sgroup);
            }
        ...
        /** Handles cases when there is FixedNum before this during disconnect.
         *
         */
        void handleBeforeFixedNum() {
            Sq before = this;
            boolean fixedNum = false;
            while (before != null) {
                if (before.hasFixedNum()) {
                    fixedNum = true;
                }
                before = before.predecessor();
            }
            if (!fixedNum) {
                Sq curr = this;
                while (curr != null) {
        ...
        /** Handles cases when there is FixedNum after this during disconnect.
         *
         * @param next the successor of this
         */
        void handleAfterFixedNum(Sq next) {
            Sq after = next;
            boolean fixedNum = false;
            while (after != null) {
                if (after.hasFixedNum()) {
                    fixedNum = true;
                }
                after = after.successor();
            }
            if (!fixedNum) {
                Sq curr = next;
                while (curr != null) {
        ...
        void disconnect() {
            ...
            if (_sequenceNum == 0) {
                ...
                handleBeforeFixedNum();
                handleAfterFixedNum(next);
            }
            Sq temp = next;
            while (temp != null) {
                temp._head = next;
